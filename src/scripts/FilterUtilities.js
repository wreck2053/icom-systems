// Create Checkboxes of all brands (plain HTML, no react-bootstrap)
...
// Return styled filter parameter tags
export function displayFilterParams(filter) {
  ...
  for (const [category, values] of Object.entries(filter)) {
    if (values && (typeof values === 'object' ? Object.values(values).some(value => value) : values)) {
      const filteredValues = typeof values === 'object'
        ? Object.keys(values).filter(key => values[key])
        : [values];

      displayItems.push(
        <span key={category} className="filter-tag">
          <span className="filter-tag-label">{category}:</span> {filteredValues.join(', ')}
        </span>
      );
    }
  }
